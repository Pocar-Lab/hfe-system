#!/usr/bin/env python3
"""Render control/monitoring PDF figures from editable sources.
...
def approx_width(text: str, size: float) -> float:
    return sum(HELVETICA_WIDTHS.get(ch, 556) * size / 1000.0 for ch in text)
...
def build() -> list[Path]:
    output_names = {f"{figure['slug']}.pdf" for figure in FIGURES}
    for old_path in OUT_DIR.iterdir():
        if old_path.name in output_names:
            old_path.unlink()
    outputs: list[Path] = []
